...
class DownloadBostonDatasetViaScraper extends Command
{
    ...
    protected $description = 'Downloads specific or all datasets from Boston Open Data directly into local storage.';
    ...
    public function handle(): int
    {
        ...
        $this->info('Boston datasets download attempted for selected datasets.');
    ...
    protected function downloadDataset($baseUrl, $resourceId, $format, $name): array
    {
        ...
        $this->info("Attempting to download Boston dataset directly: {$name} from {$datasetUrl} to {$destination}...");
        ...
        $downloadResult = $this->downloadFileDirectly($datasetUrl, $destination);
        ...
        if ($downloadResult['success']) {
            ...
            $this->error("Failed to download Boston dataset: {$name} (Resource ID: {$resourceId}) from {$datasetUrl}");
    ...
    private function downloadFileDirectly(string $datasetUrl, string $destination): array
    {
        try {
            $response = Http::timeout(600)
                ->withHeaders([
                    'User-Agent' => 'PublicDataWatch ingestion downloader',
            ...
            if (!$response->successful()) {
                ...
                $this->error("Boston dataset request for {$datasetUrl} failed. Status: {$response->status()}. Body: " . $response->body());

                return ['success' => false, 'bytes_written' => 0, 'message' => $message];
            }

            $fileContents = $response->body();

            if (empty($fileContents)) {
                $this->error("Downloaded file from {$datasetUrl} is empty.");
            ...
            $this->error("HTTP Request Exception while downloading {$datasetUrl}: " . $e->getMessage());
            return ['success' => false, 'bytes_written' => 0, 'message' => $e->getMessage()];
        } catch (\Exception $e) {
            $this->error("Error downloading file from {$datasetUrl}: " . $e->getMessage());
            return ['success' => false, 'bytes_written' => 0, 'message' => $e->getMessage()];
        }
    }
