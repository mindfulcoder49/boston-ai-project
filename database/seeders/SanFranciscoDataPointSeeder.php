...
class SanFranciscoDataPointSeeder extends Seeder
{
    private const DAYS_TO_KEEP = 183; // Approx 6 months, matches the recent data import
    private const BATCH_SIZE = 1000;

    /**
     * Define the Mappable models to process for SanFrancisco.
     */
    private const MODELS_TO_PROCESS = [
        \App\Models\SanFranciscoCrime::class,
        // Add other SanFrancisco models here in the future
    ];

    public function run()
    {
        $this->command->info("Starting SanFranciscoDataPointSeeder...");
        Log::info("SanFranciscoDataPointSeeder: Run started.");
        $cutoffDate = Carbon::now()->subDays(self::DAYS_TO_KEEP)->toDateTimeString();

        // Clean up old data points from the san_francisco_data_points table
        try {
            $deletedCount = DB::connection('san_francisco_db')->table('san_francisco_data_points')->where('alcivartech_date', '<', $cutoffDate)->delete();
            $this->command->info("Successfully deleted {$deletedCount} old SanFrancisco data points.");
            Log::info("SanFranciscoDataPointSeeder: Deleted {$deletedCount} old SanFrancisco data points.");
        } catch (\Exception $e) {
            $this->command->error("Error deleting old SanFrancisco data points: " . $e->getMessage());
            Log::error("SanFranciscoDataPointSeeder: Error deleting old SanFrancisco data points.", ['exception' => $e]);
        }
        ...
        $this->command->info("SanFranciscoDataPointSeeder finished.");
        Log::info("SanFranciscoDataPointSeeder: Run finished.");
    }
    ...
    private function syncDataPointsForModel(string $modelClass, string $cutoffDate)
    {
        ...
        $this->command->info("Processing SanFrancisco data for model: {$humanName} (table: {$sourceTableName})");
        ...
        try {
            ...
            // The source table is in the 'san_francisco_db' connection
            DB::connection('san_francisco_db')->table($sourceTableName)
                ...
                ->chunkById(self::BATCH_SIZE, function ($newDataChunk) use (
                    ...
                    if (!empty($batchInsert)) {
                        DB::connection('san_francisco_db')->table('san_francisco_data_points')->upsert(
                            $batchInsert,
            ...
            Log::error("SanFranciscoDataPointSeeder: Failed to process data for model '{$modelClass}'.", ['exception' => $e]);
        }
    }
}
