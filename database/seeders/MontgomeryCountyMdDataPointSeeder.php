...
class MontgomeryCountyMdDataPointSeeder extends Seeder
{
    private const DAYS_TO_KEEP = 183; // Approx 6 months, matches the recent data import
    private const BATCH_SIZE = 1000;

    /**
     * Define the Mappable models to process for MontgomeryCountyMd.
     */
    private const MODELS_TO_PROCESS = [
        \App\Models\MontgomeryCountyMdCrime::class,
        // Add other MontgomeryCountyMd models here in the future
    ];

    public function run()
    {
        $this->command->info("Starting MontgomeryCountyMdDataPointSeeder...");
        Log::info("MontgomeryCountyMdDataPointSeeder: Run started.");
        $cutoffDate = Carbon::now()->subDays(self::DAYS_TO_KEEP)->toDateTimeString();

        // Clean up old data points from the montgomery_county_md_data_points table
        try {
            $deletedCount = DB::connection('montgomery_county_md_db')->table('montgomery_county_md_data_points')->where('alcivartech_date', '<', $cutoffDate)->delete();
            $this->command->info("Successfully deleted {$deletedCount} old MontgomeryCountyMd data points.");
            Log::info("MontgomeryCountyMdDataPointSeeder: Deleted {$deletedCount} old MontgomeryCountyMd data points.");
        } catch (\Exception $e) {
            $this->command->error("Error deleting old MontgomeryCountyMd data points: " . $e->getMessage());
            Log::error("MontgomeryCountyMdDataPointSeeder: Error deleting old MontgomeryCountyMd data points.", ['exception' => $e]);
        }
        ...
        $this->command->info("MontgomeryCountyMdDataPointSeeder finished.");
        Log::info("MontgomeryCountyMdDataPointSeeder: Run finished.");
    }
    ...
    private function syncDataPointsForModel(string $modelClass, string $cutoffDate)
    {
        ...
        $this->command->info("Processing MontgomeryCountyMd data for model: {$humanName} (table: {$sourceTableName})");
        ...
        try {
            ...
            // The source table is in the 'montgomery_county_md_db' connection
            DB::connection('montgomery_county_md_db')->table($sourceTableName)
                ...
                ->chunkById(self::BATCH_SIZE, function ($newDataChunk) use (
                    ...
                    if (!empty($batchInsert)) {
                        DB::connection('montgomery_county_md_db')->table('montgomery_county_md_data_points')->upsert(
                            $batchInsert,
            ...
            Log::error("MontgomeryCountyMdDataPointSeeder: Failed to process data for model '{$modelClass}'.", ['exception' => $e]);
        }
    }
}
