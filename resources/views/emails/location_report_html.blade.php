@php
    ...
    $badgeGeometry = function (array $incident): array {
        ...
        return match ($shape) {
            'circle' => ['width' => 42, 'height' => 42, 'radius' => '999px', 'rotate' => true],
            'square' => ['width' => 42, 'height' => 42, 'radius' => '8px', 'rotate' => true],
            'pill' => ['width' => 52, 'height' => 38, 'radius' => '999px', 'rotate' => true],
            'bevel' => ['width' => 52, 'height' => 38, 'radius' => '16px 6px 16px 6px', 'rotate' => true],
            'tag' => ['width' => 52, 'height' => 38, 'radius' => '6px 16px 6px 16px', 'rotate' => true],
            'diamond' => ['width' => 36, 'height' => 36, 'radius' => '8px', 'rotate' => false],
            default => ['width' => 42, 'height' => 42, 'radius' => '14px', 'rotate' => true],
        };
    };
...
<body style="margin:0; padding:16px 0; background:#edf2f7; color:#1f2937; font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;">
        ...
        <tr>
            <td align="center" class="email-padding" style="padding:0 12px;">
                <table role="presentation" class="email-shell" cellpadding="0" cellspacing="0" style="border-collapse:collapse; background:#ffffff; border:1px solid #d7dde5; border-radius:18px;">
                    <tr>
                        <td class="email-padding" style="padding:24px 20px 10px; font-size:28px; font-weight:800; color:#0f172a;">
                            PublicDataWatch
                        </td>
                    </tr>
                    ...
                    @if (!empty($introNotice))
                        <tr>
                            <td class="email-padding" style="padding:0 20px 18px;">
                    ...
                    @if ($recentSnapshot)
                        <tr>
                            <td class="email-padding" style="padding:0 20px 6px; font-size:16px; line-height:1.6; color:#64748b;">
                                Most recent map for {{ $windowDisplay }} around {{ $location->address }}.
                            </td>
                        </tr>

                        <tr>
                            <td class="email-padding" style="padding:0 20px 22px;">
                                <div style="font-size:24px; line-height:1.2; font-weight:800; color:#0f172a;">
                                    {{ $windowDisplay }}
                                </div>
                                <div style="margin-top:8px; font-size:17px; line-height:1.6; color:#475569;">
                                    @if ($selectedPoints > 0)
                                ...
                                <div style="margin-top:14px; font-size:16px; line-height:1.6; color:#64748b;">
                                    Numbered badges in the map match the incidents below.
                                </div>

                                @if (!empty($incidents))
                                    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse; width:100%; margin-top:8px;">
                                        @foreach ($incidents as $incident)
                                            <tr class="summary-row">
                                                <td class="summary-badge-cell" width="58" valign="top" style="width:58px; padding:16px 10px 16px 0;">
                                                ...
                                                <td class="summary-copy" valign="top" style="padding:16px 0; color:#1f2937;">
                                                    <div style="font-weight:800; color:#0f172a;">
                                                        {{ $incident['headline'] }}
                                                    </div>
                                                    @if (!empty($incident['detail']))
                                                        <div style="margin-top:4px; color:#0f172a;">
                                                            <strong>Description:</strong> {{ $incident['detail'] }}
                                                        </div>
                                                    @endif
                                    ...
                                    @if ($omittedPoints > 0)
                                        <div style="margin-top:12px; font-size:16px; line-height:1.6; color:#64748b;">
                                            {{ $omittedPoints }} additional incident{{ $omittedPoints === 1 ? '' : 's' }} happened that day but were not shown on the map.
                                        </div>
                                    @endif
                                @else
                                    <div style="margin-top:12px; font-size:17px; line-height:1.6; color:#475569;">
                    ...
                    @elseif (!empty($publicMapsUrl))
                        <tr>
                            <td class="email-padding" style="padding:0 20px 22px;">
                                <div style="font-size:18px; line-height:1.6; color:#475569; margin-bottom:16px;">
                    ...
                    <tr>
                        <td class="email-padding" style="padding:4px 20px 28px;">
